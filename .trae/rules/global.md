...
- 用户描述的每一个需求点**必须**完整实现，禁止以"简化""缩减""图省事"为由删减或跳过。
- 遇到需求不明确或表述概括时，**必须先追问确认**（使用 AskUserQuestion），不得自行决定不做或做简化版。
- 交付前必须自查：用户提的每个需求点是否都有对应实现，不得用 TODO / 占位符 / 空实现敷衍。
- **原因**：AI 倾向于"少做少错"以规避违规风险，但这会导致交付不符合预期，返工成本远高于多问一句的成本。
...
### 8.1 最小改动 vs 需求完整
- **必须完整实现用户描述的所有需求点**，不得以"简化""缩减""图省事"为由删减需求。
- 遇到需求不明确或表述概括时，必须先追问确认，不得自行决定不做或做简化版。
- 不要顺手重构与当前需求无关的代码。
- 不要添加用户未明确要求的"优化"或"额外功能"。
- "最小改动"仅指：不顺手重构无关代码、不添加未要求的优化；**不等于可以少做需求**。
- **原因**：个人开发阶段，过度重构会增加不可预知的 Bug 风险；但缩减需求会导致交付不符合预期，返工成本更高。
