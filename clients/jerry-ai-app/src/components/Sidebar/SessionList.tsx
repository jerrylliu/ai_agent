...
const SessionList: React.FC<SessionListProps> = ({
  ...
  // 菜单 Portal 位置：在省略号右侧弹出，底部与会话卡片底部对齐，不遮挡左侧栏内容
  const [menuPosition, setMenuPosition] = React.useState<{ bottom: number; left: number } | null>(null);
  ...
  // 打开菜单：在省略号右侧弹出，底部与会话卡片底部对齐
  const openMenu = (sessionId: string, buttonEl: HTMLButtonElement) => {
    const rect = buttonEl.getBoundingClientRect();
    // 找到会话卡片（按钮的祖先中带 cursor-pointer 的那个）
    const card = buttonEl.closest('[class*="cursor-pointer"]') as HTMLElement;
    const cardRect = card?.getBoundingClientRect() || rect;

    setMenuPosition({
      bottom: window.innerHeight - cardRect.bottom,
      left: rect.right + 4, // 紧贴省略号右侧，留 4px 间距
    });
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4">
                    ...
                    <Button
                      ...
                      onClick={(e) => {
                        ...
                        if (menuSessionId === session.sessionId) {
                          ...
                          openMenu(session.sessionId, e.currentTarget);
                        }
                      }}
                      title="更多操作"
                    >
      ...
      {/* ==================== Portal 下拉菜单（渲染到 body，避免被父容器 overflow 裁切） ==================== */}
      {menuSessionId && menuPosition && createPortal(
        <div
          className="fixed z-[90] bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-lg shadow-xl py-1 min-w-[160px]"
          style={{ bottom: menuPosition.bottom, left: menuPosition.left }}
          onClick={(e) => e.stopPropagation()}
        >
          {(() => {
            const session = sessions.find(s => s.sessionId === menuSessionId);
