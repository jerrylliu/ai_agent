...
interface ModelPanelProps {
  ...
  onProbeCapabilities: () => Promise<{ success: boolean; message: string }>;
  ...
  return (
    <div className="absolute right-0 top-0 h-full z-20">
      ...
      >
        ...
        {/* pr-[var(--safe-right)]：平板横屏时系统导航栏在右侧，面板紧贴右边缘会被压住 */}
        <div className="w-64 h-full flex flex-col pt-[var(--safe-top)] pb-[calc(var(--safe-bottom)+var(--safe-keyboard))] pr-[var(--safe-right)]">
          {/* 面板头部 */}
          ...
          <div className="px-4 py-3 border-b border-gray-100 dark:border-slate-600 bg-blue-50/50 dark:bg-blue-900/20">
            <p className="text-xs text-gray-500 dark:text-gray-300 mb-1 cyberpunk-model-current-label">
              当前模型
            </p>
            <div className="flex items-center space-x-2">
          ...
          <div className="flex-1 min-h-0 overflow-y-auto">
                  ...
                  .map((model) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <p className="text-sm text-gray-900 dark:text-white cyberpunk-model-item-name">
                          {model.name}
                        </p>
                        {currentModelId === model.id && (
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-300 mt-0.5 cyberpunk-model-item-desc">
                        {model.description}
                      </p>
                    </button>
                  ))}
            ...
            {/* DeepSeek 线上模型列表 */}
            {availableModels.filter((m) => m.provider === "deepseek").length >
              0 && (
            ...
            {/* 智谱线上模型列表 */}
            {availableModels.filter((m) => m.provider === "zhipu").length >
              0 && (
              <div className="px-3 py-2 border-t border-gray-100 dark:border-slate-600">
                <p className="text-xs font-medium text-gray-400 dark:text-gray-400 px-1 mb-1 flex items-center cyberpunk-model-group-label">
          ...
          <div className="px-3 py-2 border-t border-gray-100 dark:border-slate-600">
            <button
              className="w-full px-3 py-2 text-left text-sm text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg flex items-center transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={probing}
              onClick={async () => {
                setProbing(true);
                const result = await onProbeCapabilities();
                setProbing(false);
                onAlert(result.message);
              }}
            >
